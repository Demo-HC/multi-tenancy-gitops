...
install_argocd () {
    ...
    pushd ${OUTPUT_DIR}
    oc create ns openshift-gitops || true
    oc apply -f gitops-0-bootstrap/setup/ocp4x/
    while ! oc wait crd applications.argoproj.io --timeout=-1s --for=condition=Established  2>/dev/null; do sleep 30; done
    sleep 60
    while ! oc wait pod --timeout=30s --for=condition=Ready --all -n openshift-gitops > /dev/null; do sleep 30; done
    popd
}
...
argocd_git_override () {
  ...
  sleep 5
  oc wait pod --timeout=-1s --for=condition=Ready --all -n openshift-gitops > /dev/null
}
...
set_git_source () {
  ...
  GIT_ORG=${GIT_ORG} GIT_GITOPS_NAMESPACE=openshift-gitops source ./scripts/set-git-source.sh
  if [[ ${GIT_TOKEN} ]]; then
...
set-git-cp4s-pattern () {
  ...
  # GIT_ORG=${GIT_ORG} GIT_GITOPS_NAMESPACE=openshift-gitops source ./scripts/set-git-cp4d-healthcare-pattern.sh
  # if [[ ${GIT_TOKEN} ]]; then
  #   git remote set-url origin ${GIT_PROTOCOL}://${GIT_TOKEN}@${GIT_HOST}/${GIT_ORG}/${GIT_GITOPS}
  # fi
